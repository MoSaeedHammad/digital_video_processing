...
def MotionEstimation(Ch1,Ch2,x,y,s):
    ...
    Step1 = [Ch2[x:x+s,y:y+s], Ch2[x:x+s,y+(4*s):y+(4*s)+s], Ch2[x-(4*s):x-(4*s)+s,y:y+s], Ch2[x:x+s,y-(4*s):y-(4*s)+s], Ch2[x+(4*s):x+(4*s)+s,y:y+s], Ch2[x+(4*s):x+(4*s)+s,y+(4*s):y+(4*s)+s], Ch2[x+(4*s):x+(4*s)+s,y-(4*s):y-(4*s)+s], Ch2[x-(4*s):x-(4*s)+s,y-(4*s):y-(4*s)+s], Ch2[x-(4*s):x-(4*s)+s,y+(4*s):y+(4*s)+s]]
    for i in range(0,9): # Using a grid of (9) indices & 4 steps block search
        ...
        if (Sad<SAD):
            ...
            if i == 5: xi, xj = x+(4*s), y+(4*s)
            if i == 6: xi, xj = x+(4*s), y-(4*s)
            if i == 7: xi, xj = x-(4*s), y-(4*s)
            if i == 8: xi, xj = x-(4*s), y+(4*s)
            Step2 = [Ch2[xi:xi+s,xj+(2*s):xj+(2*s)+s], Ch2[xi-(2*s):xi-(2*s)+s,xj:xj+s], Ch2[xi:xi+s,xj-(2*s):xj-(2*s)+s], Ch2[xi+(2*s):xi+(2*s)+s,xj:xj+s], Ch2[xi+(2*s):xi+(2*s)+s,xj+(2*s):xj+(2*s)+s], Ch2[xi+(2*s):xi+(2*s)+s,xj-(2*s):xj-(2*s)+s], Ch2[xi-(2*s):xi-(2*s)+s,xj-(2*s):xj-(2*s)+s], Ch2[xi-(2*s):xi-(2*s)+s,xj+(2*s):xj+(2*s)+s]]
            for j in range(0,8): # Using a grid of (8) indices & 2 steps block search
                ...
                if (Sad<SAD):
                    ...
                    if j == 4: xi, xj = xi+(2*s), xj+(2*s)
                    if j == 5: xi, xj = xi+(2*s), xj-(2*s)
                    if j == 6: xi, xj = xi-(2*s), xj-(2*s)
                    if j == 7: xi, xj = xi-(2*s), xj+(2*s)
                    Step3 = [Ch2[xi:xi+s,xj+(s):xj+(s)+s], Ch2[xi-(s):xi-(s)+s,xj:xj+s], Ch2[xi:xi+s,xj-(s):xj-(s)+s], Ch2[xi+(s):xi+(s)+s,xj:xj+s], Ch2[xi+(s):xi+(s)+s,xj+(s):xj+(s)+s], Ch2[xi+(s):xi+(s)+s,xj-(s):xj-(s)+s], Ch2[xi-(s):xi-(s)+s,xj-(s):xj-(s)+s], Ch2[xi-(s):xi-(s)+s,xj+(s):xj+(s)+s]]
                    for k in range(0,8): # Using a grid of (4) indices & 1 step block search
                        ...
                        if (Sad<SAD):
                            ...
                            if k == 4: xi, xj = xi+(s), xj+(s)
                            if k == 5: xi, xj = xi+(s), xj-(s)
                            if k == 6: xi, xj = xi-(s), xj-(s)
                            if k == 7: xi, xj = xi-(s), xj+(s)
    Mv = [xi,xj]
    return Mv
#%%
